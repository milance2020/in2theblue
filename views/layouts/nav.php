...
<head>
    ...
    >
    <meta charset="UTF-8">
    <title><?= htmlspecialchars($_output['meta_title'] ?? 'Shop') ?></title>

    <meta name="description" content="<?= htmlspecialchars($_output['meta_description'] ?? '') ?>">

    <meta name="robots" content="<?= htmlspecialchars($_output['meta_robots'] ?? 'index, follow') ?>">

    <?php if (!empty($_output['canonical'])): ?>
        <link rel="canonical" href="<?= htmlspecialchars($_output['canonical']) ?>">
    <?php endif; ?>
